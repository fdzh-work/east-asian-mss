...
declare variable $countryauthorities := doc('../authority/places.xml')/tei:TEI/tei:text/tei:body//tei:listPlace/tei:place[@xml:id and @type='country'];
declare variable $worksauthority := doc("../authority/works.xml");

declare function local:origin($countrykeyatts as attribute()*, $solrfield as xs:string) as element()*
{
    (: Lookup place keys, which are specific to east-asian-mss :)
...
declare function local:buildSummary($msdescorpart as element()) as xs:string
{
    ...
    let $head := local:convertlb($msdescorpart/tei:head/node())    
